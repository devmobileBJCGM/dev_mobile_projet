<template>
    ...
    <button v-on:click="submit_getUserData" class=" block uppercase mx-auto shadow px-4 mt-4 transition duration-500 ease-in-out bg-blue-400 hover:bg-red-600 transform hover:-translate-y-1 hover:scale-110 text-white text-xs py-3 px-10 rounded ">Recharger vos données</button>
    ...
    <div id="Todo" class="flex flex-wrap overflow-hidden divide-x-4 divide-gray ">
        ...
        <div id="List_task" class="w-full overflow-hidden xl:w-1/2">
            <div id="filters" class="mt-10 relative inline-flex mb-5 space-x-2 ">
                <a id="filter1"  class="focus:ring-2 focus:ring-red-800 rounded shadow-md block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900" href="#" :class="{selected: filter === 'all'}" v-on:click="filter = 'all'">Toutes</a>
                <a id="filter2" class="focus:ring-2 focus:ring-red-800 rounded shadow-md block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900" href="#" :class="{selected: filter === 'todo'}" v-on:click="filter = 'todo'">A faire</a>
                <a id="filter3" class="focus:ring-2 focus:ring-red-800 rounded shadow-md block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900" href="#" :class="{selected: filter === 'done'}" v-on:click="filter = 'done'">Faites</a>
            </div>
            <br>
            ...
            <div v-if="this.idTodoSelected != -1">
                <div class="  ml-24 flex mb-5 space-x-2">
                    <div class="mt-4 mr-28 flex items-center">
                ...
                <button class="  mt-4 rounded bg-indigo-600 focus:ring-2 focus:ring-indigo-800 shadow-md block px-4 py-2 text-sm text-gray-100 hover:bg-indigo-500" v-on:click="showEdit()">Modifier tâches</button>
                <button class=" mt-4 rounded bg-indigo-900 focus:ring-2 focus:ring-indigo-800 shadow-md block px-4 py-2 text-sm text-gray-100 hover:bg-indigo-500" v-on:click="deleteCompleted">Supprimer les tâches finies</button>
