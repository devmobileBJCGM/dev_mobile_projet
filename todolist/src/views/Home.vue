<template>
    ...
    <button @click.prevent="submit_getUserData" class="block uppercase mx-auto shadow px-4 mt-4 transition duration-500 ease-in-out bg-blue-400 hover:bg-blue-600 transform hover:-translate-y-1 hover:scale-110 text-white text-xs py-3 px-10 rounded ">Recharger vos données</button>
    ...
    <div id="Todo" class="flex flex-wrap overflow-hidden divide-x-4 divide-gray ">
        <div id="List_todo" class="w-full overflow-hidden xl:w-1/2" >
            ...
            <button class="transform hover:scale-110 transition duration-500 ease-in-out focus:outline-none text-black-500"  @click.prevent="createTodoList"><svg width="25" height="25" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></button>
            </div>
        </div>
        <div id="List_task" class="w-full overflow-hidden xl:w-1/2">
            <div id="filters" class="mt-10 relative inline-flex mb-5 space-x-2 ">
                <a id="filter1"  class="focus:ring-2 focus:ring-red-800 rounded shadow-md block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900 border-2 border-black-300" href="#" :class="{selected: filter === 'all'}" @click.prevent="filter = 'all'">Toutes</a>
                <a id="filter2" class="focus:ring-2 focus:ring-red-800 rounded shadow-md block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900 border-2 border-black-300" href="#" :class="{selected: filter === 'todo'}" @click.prevent="filter = 'todo'">A faire</a>
                <a id="filter3" class="focus:ring-2 focus:ring-red-800 rounded shadow-md block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900 border-2 border-black-300" href="#" :class="{selected: filter === 'done'}" @click.prevent="filter = 'done'">Faites</a>
            </div>
            <br>
            ...
            <ul class="mx-24 pb-4  focus:outline-none ease-in-out bg-white-600   rounded py-2 px-3 border-black-500 border-2">
                <li class="" v-for="todo in filteredTodos" :key="todo.id" :style="colorStyle(todo.completed)">
                    <div class="flex items-baseline justify-start space-y-2  ">
                        <input class="mr-6" type="checkbox" v-model="todo.completed" @click.prevent="changeBool(todo)">
                    <listask class="" :id="todo.id" :completed="todo.completed==1?true:false" :task="todo.name"></listask>
                        ...
                        <button v-if="edit" class="focus:outline-none text-black text-sm py-2.5 px-1 rounded-md bg-black-500  hover:bg-black-600 hover:shadow-lg" @click.prevent="modifyTodoNameAction(todo)"><svg width="20" height="20" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" /></svg></button>
                        <button class="focus:outline-none text-black text-sm py-2  px-1 rounded-md  hover:bg-red-600 hover:shadow-lg" @click.prevent="deleteTodoElement(todo.id)"> <svg width="20" height="20" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg> </button>
            ...
            <div v-if="this.idTodoSelected != -1">
                <div class="  ml-24 flex mb-5 space-x-2">
                    <div class="mt-4 flex items-center">
                        ...
                        <button  class="transform hover:scale-110 transition duration-500 ease-in-out focus:outline-none rounded-full bg-black-500 hover:bg-black-600 hover:shadow-lg" @click.prevent="createTodo(this.idTodoSelected)"><svg width="25" height="25" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></button>
                    </div>
                    
                    <button class="  mt-4 rounded bg-indigo-800 hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-800 shadow-md block px-4 py-2 text-sm text-gray-100 " @click.prevent="showEdit()">Modifier tâches</button>
                    <button class=" mt-4 rounded bg-indigo-800 hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-800 shadow-md block px-4 py-2 text-sm text-gray-100" @click.prevent="deleteCompleted">Supprimer les tâches finies</button>
                </div>
            </div>
            <br>
        </div>
